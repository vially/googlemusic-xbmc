...
class Actions:
    ...
    def executeAction(self, action, params):
        if action == "play_all":
            utils.playAll(self._getSongs(params), 'shuffle' in params, params.get('song_id'))
            ...
            self.api.getPlaylistsByType(params["playlist_type"])
            ...
            keyboard = xbmc.Keyboard(params['display_name'], self.lang(30402))
            keyboard.doModal()
            ...
            xbmc.executebuiltin("ActivateWindow(10025,plugin://plugin.video.youtube/search/?q=%s)" % params['display_name'])
        elif action == "play_yt":
            self.playYoutube([params.get('display_name')])
        elif action == "search":
            xbmc.executebuiltin(
    ...
    def notify(self, text):
        xbmc.executebuiltin("XBMC.Notification(%s,%s,5000,%s)" % (utils.plugin, utils.tryEncode(text), utils.addon.getAddonInfo('icon')))
    ...
    def addToPlaylist(self, song_id):
        ...
        plist = [pl_name for pl_id, pl_name, pl_arturl, token, recent in playlists]
    ...
    def setThumbs(self, song_id):
        ...
        if selected >= 0:
            thumbs = {'0': '5', '1': '0', '2': '1'}[str(selected)]
            self.api.setThumbs(song_id, thumbs)
            self.notify(self.lang(30110))

    def addFavourite(self, name, params):
        import fileinput
        path = os.path.join(xbmc.translatePath("special://masterprofile").decode('utf-8'), "favourites.xml")
        ...
        fav = fav % (name, xbmc.translatePath(utils.addon.getAddonInfo('icon')), utils.addon_url, url[5:])
    ...
    def _getSongs(self, params):
        ...
        if get('playlist_id'):
            ...
            songs = self.api.getSharedPlaylist(get('share_token'))
        elif get('artist_id'):
    ...
    def _getVideoIDs(self, titles, progress_dialog=None):
        ...
        params = {'part': 'id',
                  'maxResults': 1,
                  'type': 'video',
                  'order': 'viewCount',  # 'regionCode':'us', #'safeSearch': 'none',
        ...
        for title in titles:
            ...
            req = Request(url % urllib.urlencode(params), headers=headers)
            response = urlopen(req).read()
            utils.log(title + "|" + repr(response))
            searchresults = json.loads(response)['items']
            if searchresults:
